...
export const fetchListUserAsync = createAsyncThunk(
  ...
  async () => {
    try {
      const response: ListUserResponse = await requestGetUserInfo({
        page: 1,
        limit: LIMIT_ITEM_PER_PAGE,
      })
      if (response.data.length > 0) {
        for (let i = 0; i < response.data.length; i++) {
          const responseFull: UserItemFullResponseProps =
            await requestGetUserInfoFull(response.data[i].id)
